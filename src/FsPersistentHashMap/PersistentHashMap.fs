...
#if PHM_USE_INTRINSICS
    // Use intrinsics
    System.Runtime.Intrinsics.X86.Popcnt.PopCount (uint32 i)
#else
...
type [<AbstractClass>] PersistentHashMap<'K, 'V when 'K :> System.IEquatable<'K>>() =
  ...
  member x.Visit   v  = x.DoVisit (OptimizedClosures.FSharpFunc<_, _, _>.Adapt v)
  member x.Set     k v=
    let h = hashOf k
